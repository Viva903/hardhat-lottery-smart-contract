...
!developmentChains.includes(network.name)
    ? describe.skip
    : describe("Raffles Unit Tests", async function () {
          ...
          describe("constructor", function () {
              it("initializes the raffle correctly", async function () {
                  // ideally we want to have just 1 asser per "it"
          ...
          describe("enterRaffle", function () {
              ...
              it("does not allow entrance when raffle is in calculating state", async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
                  // pretend to be a Chainlin Keeper
                  ...
                  await new Promise(async (resolve, reject) => {
                      raffle.once("WinnerPicked", async () => {
                          ...
                          try {
                              ...
                              console.log(`Total Entrants : ${additionalEntrants + 1}`)
                              console.log(
                                  `Additional Lottery Prize : ${raffleEntranceFee
                                      .mul(additionalEntrants)
                                      .add(raffleEntranceFee)}`
                              )
                              console.log(
                                  `Calculated Winner Ending Balance : ${winnerStartingBalance.add(
                                      raffleEntranceFee
                                          .mul(additionalEntrants)
                                          .add(raffleEntranceFee)
                                  )}`
                      ...
                      // we will fire the event, the listener will pick it up, and resolve
                      const tx = await raffle.performUpkeep([])
